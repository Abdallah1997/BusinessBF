  ...
  return (
    <>
      ...
      {params.error === "ebay-auth-failed" && (
        <Card className="mb-6 border-red-200 bg-red-50/60 p-4">
          <p className="text-sm text-red-700">eBay connection failed. Start the flow again.</p>
        </Card>
      )}
          ...
          {!ebayOn && (
            <div className="mt-4 rounded-lg border border-dashed border-zinc-300 bg-zinc-50/60 p-4">
              <p className="text-sm text-zinc-600">
                ...
                redirect URL to <code className="rounded bg-zinc-100 px-1">/api/ebay/callback</code>, then add{" "}
                <code className="rounded bg-zinc-100 px-1">EBAY_CLIENT_ID</code>,{" "}
                <code className="rounded bg-zinc-100 px-1">EBAY_CLIENT_SECRET</code> and{" "}
                <code className="rounded bg-zinc-100 px-1">EBAY_RU_NAME</code> to .env and restart. A Connect button
                appears here.
              </p>
            </div>
          )}
          ...
          {ebayConn?.status === "CONNECTED" && (
            <div className="mt-5 grid grid-cols-1 gap-6 lg:grid-cols-2">
              <div className="rounded-lg border border-zinc-200 p-4">
                <h3 className="text-sm font-semibold text-zinc-900">Import listings</h3>
              ...
              <div className="rounded-lg border border-zinc-200 p-4">
                <h3 className="text-sm font-semibold text-zinc-900">Post to eBay</h3>
                <p className="mt-1 text-xs text-zinc-400">
                ...
                {sellableItems.length === 0 ? (
                  <p className="mt-3 text-xs text-zinc-400">Add inventory first.</p>
                ) : (
                  <div className="mt-3">
